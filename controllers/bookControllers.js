const bookController = (Book) => {
	...
	const postBooks = async (req, res) => {
		const book = new Book(req.body); //metodo post
	...
	const getBookByTitle = async (req, res) => {
		const { query } = req;
		const response = await Book.findByTitle({query});
		res.json(response);
	...
	return{ getBooks, postBooks, getBookById, getBookByAuthor, getBookByTitle, putBooks, deleteBooksById};
}

module.exports = bookController; //exporto o hago visible la funcion 